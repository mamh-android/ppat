...
function pre_parse(){
	cat $TARGET_LOG | grep -E '(UIAT|FPS|Displayed)' > temp.txt
	echo $TARGET_LOG | grep -q Displayed
	if [ $? -eq 0 ]
	then
		DISPLAYED=0
	else
...
function main(){
	...
	do
		...
		echo $LINE | grep -q Displayed
		if [ $? -eq 0 ]
		then
			DISPLAYED=1
		fi
		echo $LINE | grep -q FPS
		if [ $? -eq 0 -a $START -eq 1 -a $LAUNCH_OVER -eq 0 ]
		then
			FRAME_TIME=`echo $LINE | awk -F " " '{print $15}'`
			...
			then
				...
				if [ $FRAME_TIME -gt 500 -a $FRAME_TIME -le 950 -a $DISPLAYED -eq 1 ]
				then
					LAUNCH_OVER=1
				else
					parse_fps_log
